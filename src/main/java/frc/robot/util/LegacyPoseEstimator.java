 ...
 * drivetrain (e.g., {@link DifferentialDriveLegacyPoseEstimator}). It is intended to be a drop-in
 * replacement for {@link Odometry}; in fact, if you never call {@link
 * LegacyPoseEstimator#addVisionMeasurement} and only call {@link LegacyPoseEstimator#update} then this will
 * behave exactly the same as Odometry.
 *
 * <p>{@link LegacyPoseEstimator#update} should be called every robot loop.
 *
 * <p>{@link LegacyPoseEstimator#addVisionMeasurement} can be called as infrequently as you want; if you
 * never call it then this class will behave exactly like regular encoder odometry.
 *
 * @param <T> Wheel positions type.
 */
public class LegacyPoseEstimator {
    ...
    private StructPublisher<Pose2d> odoPub;



    /**
     * Constructs a LegacyPoseEstimator with default standard deviations for the model and vision
     * measurements.
     *
    ...
    /**
     * Constructs a LegacyPoseEstimator.
     *
     * @param kinematics A correctly-configured kinematics object for your drivetrain.
     * @param gyroAngle The current gyro angle.
    ...
    public LegacyPoseEstimator(
        ...
        odoPub = NetworkTableInstance.getDefault().getTable(Constants.currentRobot.toString()).getStructTopic("odometry pose", Pose2d.struct).publish();
     ...
     * LegacyPoseEstimator#update} every loop.
     *
     ...
     *     LegacyPoseEstimator#updateWithTime(double,Rotation2d,Object)} then you must use a timestamp with
     ...
     * LegacyPoseEstimator#setVisionMeasurementStdDevs(Matrix)} or this method.
     *
     * @param visionRobotPoseMeters The pose of the robot as measured by the vision camera.
     * @param timestampSeconds The timestamp of the vision measurement in seconds. Note that if you
    ...
    public synchronized Pose2d updateWithTime(double currentTimeSeconds, Rotation2d gyroAngle, SwerveModulePosition[] wheelPositions) {
        ...
        odoPub.accept(m_odometry.getPoseMeters());
    ...
    private final class InterpolationRecord implements Interpolatable<InterpolationRecord> {
         ...
         * @param wheelPositions The current encoder readings.
         */
        private InterpolationRecord(Pose2d poseMeters, Rotation2d gyro, SwerveModulePosition[] wheelPositions) {
        ...
        return this == obj
            || obj instanceof LegacyPoseEstimator.InterpolationRecord record
